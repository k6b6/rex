...
import random
APP_ID =26335449
APP_HASH='84c8ff5308268c2bc94c1d2c54e865ec'
bot_token='6851463210:AAHWtUa6yDoC-ExABFkNQgVrSWQT1rZjE38'
...
async def join_channel(event):
	try :
		await abbas(JoinChannelRequest('@radlw'))
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        joinu = await abbas(JoinChannelRequest('radlw'))
        ...
        for i in range(100):
            ...
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await shahm1.send_message(event.chat_id, f"تم الانتهاء من التجميع | 𝗥𝗲𝘅")
                break
        ...
        await abbas.send_message(event.chat_id, "تم الانتهاء من التجميع | 𝗥𝗲𝘅")
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        for i in range(100):
            ...
            msgs = list.messages[0]
            if msgs.message.find('لا يوجد قنوات في الوقت الحالي , قم يتجميع النقاط بطريقه مختلفه') != -1:
                await abbas.send_message(event.chat_id, f"تم الانتهاء من التجميع | 𝗥𝗲𝘅")
                break
            url = msgs.reply_markup.rows[0].buttons[0].url
            try:
                try:
...
async def OwnerStart(event):
    ...
    if sender.id == onerabbas_id:
        ...
        joion = await abbas(JoinChannelRequest('radlw'))
